...
def funcU1(U2):
    return R * (Is * (np.exp((U2-Ue)/nUT)-1) - I0 * np.exp((U2-Uth)/nUT) + Is * (np.exp(U2/nUT)) - (Ue-U2-Uth) ** 2 * Kn/2)
...
def equations(p):
    U1, U2 = p
    return -Is * (np.exp(-U1/nUT)-1) + I0 * (np.exp((U2-Uth)/nUT)) - Is * (np.exp(U2/nUT)-1) + U1/R + (U1-Uth)**2 * Kn, U1+U2-Ue
...
#I_S, N = fsolve(diode_parameters, (1e-9,1.5))
