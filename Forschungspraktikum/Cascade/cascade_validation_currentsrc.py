        ...
        if opamp_max_supply_vec[i] > upper_voltage_diff > opamp_min_supply_vec[i] and \
            ...
            with lock:
                ...
                with open("cascade_solutions.txt", "a", encoding="utf-8") as datei:
                    datei.write(
                        f"V_in={V_in} V; R_in={R_in} Ohm; Diode {diode}; C={C_DC} µF; Opamp = {opamp_type_vec[i]}; E={E} J; "
...
if __name__ == "__main__":
    ...
    brute_force_total_count = len(opamp_type_vec) * len(R_in_vec) * len(V_in_vec) * len(diodes) * len(Freq_vec)
            ...
            for V_in in V_in_vec:
                for diode in diodes:
                    ...
                    for freq in Freq_vec:
                        ...
                        param_list.append([i, R_in, V_in, diode, freq])
