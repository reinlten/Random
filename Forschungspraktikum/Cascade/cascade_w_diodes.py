...
C_DC_vec = [1, 2, 5, 10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 20000, 50000, 100000] #nF
#C_DC_vec = [40, 4, .4, .04]
...
overall_optimal_energies = {}

for V_in in V_in_vec: #V_in
    for R_in in R_in_vec: #R_in

        curr_energies = {}

        for diode in diodes: #Diodes
            for C_DC in C_DC_vec: #Cap sizes
                for i in range(3): #Opamps
                    ...
                    if opamp_max_supply_vec[i] > voltage_diff > opamp_min_supply_vec[i]:
                        ...
                        if min(voltage_diff[25000:]) > opamp_min_supply_vec[i] and pos_ripple < pos_max*0.1:
                            ...
                            curr_energies[f"Diode {diode}; C={C_DC}; Opamp = {opamp_type_vec[i]}"] = E
        if not curr_energies:
            print(f"No solution for V_in={V_in} and R_in={R_in} that meets the requirements")
        else:
            print(f"Best solution for V_in={V_in} and R_in={R_in}: {min(curr_energies, key=curr_energies.get)} (E={min(curr_energies.values())})")
